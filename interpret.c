...
struct value *get_one_expr(pp, from)
    struct lnode **pp;
    char *from;
{
    struct value *v;
...
static struct value *explode_string(str, del)
    char *str, *del;
{
    char *p, *beg;
    int num, len;
...
struct value *implode_string(arr, del)
    struct vector *arr;
    char *del;
{
    int size, i, num;
