	...
	*sp2=0;
	...
	if (!call_obj(QADJFUNC,on,ad)) { /* Not ok */
	  ...
	  if (ret->type==T_STRING) {
	    ...
	    if (sp2) sp=sp2;
	    sprintf(tot,"%s %s",ad,sp); lowercase(tot);
...
{ 
  ...
  ad=0; ocmd=*cmd; strcpy(gAdjective,"");
...
{ 
  int nm,s;
  OBJECT *pn;
  char *ocmd,w1[KLUDGELEN],w2[KLUDGELEN],w3[KLUDGELEN];
    ...
    char **cmd;
{
  ...
  if (gForprepos) {
    v = (VALUE *) get_one_expr(&gForprepos);
    ...
    LNODE *l;
{
  ...
  int ilop,
  altflag,             /* =1 (true) i altkedja, match har intr{ffat */
  ...
  */
  ...
  while (ptyp != EP) {
    ...
    */
	...
	ptyp=get1ps(&parse,&l,1); /* Skip this pattern */
	ptyp=get1ps(&parse,&l,0); /* Next real pattern or ALT */
      ...
      tmp=get1ps(&a,&try,0);
      if (tmp == ALT) { 
	ptyp=get1ps(&parse,&l,0); /* Skip ALT */
	ptyp=get1ps(&parse,&l,0); /* Next real pattern or ALT */
      }
      else {
...
VALUE *describe_items (arr, func, live)
    VALUE *arr;
    char *func;
    int live;
{
    ...
    int ant;
{
  ...
  if (ant == 1) {
    if EQN("the ",str) {
      strcpy(dest,"the "); strcat(dest,splitfix(&tmp[4],ant)); return dest;
    }
    if EQN("The ",str) {
      strcpy(dest,"the "); strcat(dest,splitfix(&tmp[4],ant)); return dest;
    }
