...
extern char *xalloc(), *string_copy(), *dump_trace(), *check_file_name(),
    *xalloc();
 ...
 * it.
 */
struct object *load_object(lname)
    char *lname;
{
    FILE *f;
    ...
    struct object *ob;
{
	...
	if (p > item.u.string && *p == ' ') {
	    count = atoi(p+1) - 1;
	    *p = '\0';
	    length = p - item.u.string;	/* This is never used again ! */
	}
    }
 ...
 * user input string.
 */
struct value *input_to(fun, flag)
    char *fun;
    int flag;
{
    ...
    char *path, *flags;
{
	...
	*p = '\0';
	...
	if (!do_match && de->d_name[de->d_namlen-1] == 'i' &&
	    de->d_name[de->d_namlen-2] == '.')
	    continue;
	...
	if (do_match && !match_string(regexp, de->d_name))
	    continue;
	if (de->d_namlen + pos >= sizeof name_buffer) {
	    truncated = 1;
	    break;
	}
	if (de->d_namlen > max)
...
struct value *print_file(path, start, len)
    char *path;
    int start, len;
{
    char buff[1000];
    ...
    int arg, arg2, arg3;
{
	...
	if (set_heart_beat(current_object, arg))
	    return &const1;
	else
	    return &const0;
    case F_LOG_FILE:
	...
	return 0;
    case F_LS:
	list_files((char *)arg, (char *)0);
	return 0;
    case F_CAT:
	return print_file((char *)arg, arg2, arg3);
    case F_INPUT_TO:
	if (current_object->destructed)
	     error("input_to() on destructed object.\n");
	return input_to((char *)arg, arg2);
	break;
	...
	return ret;
    case F_COMMAND:
	if (command_for_object((char *)arg, (struct object *)arg2))
	    return &const1;
	else
	    return &const0;
    case F_PRESENT:
